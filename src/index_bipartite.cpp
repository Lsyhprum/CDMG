...
void IndexBipartite::LinkBase(const Parameters &parameters) {
...
//     // prefetch_vector((char *)(data_bp_ + projection_ep_ * dimension_), dimension_); // new
//     // init_ids.push_back(projection_ep_);
...
//             // _mm_prefetch((char *)(visited_array + *(cur_nbrs + j + 1)), _MM_HINT_T0); // new
//             // _mm_prefetch((char *)(data_bp_ + *(cur_nbrs + j + 1) * dimension_), _MM_HINT_T0); // new
//             // }
...
std::pair<uint32_t, uint32_t> IndexBipartite::SearchRoarGraph(const float *query, size_t K, size_t &qid, const Parameters &parameters,
    unsigned *indices, std::vector<float>& res_dists) {
    uint32_t L = parameters.Get<uint32_t>("L_pq");
    std::vector<Neighbor> retset(L + 1);
    std::vector<unsigned> init_ids(L);
    boost::dynamic_bitset<> flags{nd_, 0};
    // NeighborPriorityQueue search_queue(L_pq); // new
    // search_queue.reserve(L_pq);
    // std::random_device rd;
    // std::mt19937 gen(rd());
    // std::uniform_int_distribution<uint32_t> dis(0, u32_nd_ - 1);
    ...
    // uint32_t start = projection_ep_;
    // projection_ep_ = start;
    // std::vector<uint32_t> init_ids;//new
    // init_ids.push_back(projection_ep_);//new
    // prefetch_vector((char *)(data_bp_ + projection_ep_ * dimension_), dimension_); // new
    // init_ids.push_back(projection_ep_);
    // _mm_prefetch((char *)data_bp_ + projection_ep_ * dimension_, _MM_HINT_T0);
    // init_ids.push_back(dis(gen));
    ...
    // std::bitset<bsize> visited;
    // block_metric.record();
    // VisitedList *vl = visited_list_pool_->getFreeVisitedList(); //new
    // vl_type *visited_array = vl->mass;//new
    // vl_type visited_array_tag = vl->curV;//new
    
    unsigned tmp_l = 0;
    for (; tmp_l < L && tmp_l < projection_graph_[projection_ep_].size(); tmp_l++) {
      init_ids[tmp_l] = projection_graph_[projection_ep_][tmp_l];
    ...
    for (unsigned i = 0; i < init_ids.size(); i++) {
        ...
        float dist =
            distance_->compare(data_bp_ + dimension_ * id, query, (unsigned)dimension_);
        // NDC++;
        cmps ++;
    ...
    while (k < (int)L) {
        ...
        if (retset[k].flag) {
          ...
          for (unsigned m = 0; m < projection_graph_[n].size(); ++m) {
            unsigned id = projection_graph_[n][m];
            if (flags[id]) continue;
            flags[id] = 1;
            float dist =
                distance_->compare(query, data_bp_ + dimension_ * id, (unsigned)dimension_);
            // NDC++;
            cmps++;
    ...
    for (size_t i = 0; i < K; i++) {
        ...
        // res_dists[i] = search_queue[i].distance;
    // std::cout << retset[i].distance << " ";
    }

    // for (auto &id : init_ids) { //new
