...
void eval_recall(std::vector<std::vector<unsigned>>& res, std::vector<std::vector<unsigned> > &gt, unsigned K){
    ...
    std::cerr << "[recall@" << K << "]: " << (mean_acc * 1.0f / res.size()) * 100 << std::endl;
}  
...
int main(int argc, char **argv) {
    ...
    std::cerr << "[base file]: " << base_data_file << std::endl;
    std::cerr << "[base shape]: " << base_num << " " << base_dim << std::endl;
    ...
    // float *aligned_query_data = efanna2e::data_align(query_data, q_pts, q_dim); //  new
    std::cerr << "[query file]: " << query_file << std::endl;
    std::cerr << "[query shape]: " << q_pts << " " << q_dim << std::endl;
    ...
    std::cerr << "[groundtruth file]: " << gt_file << std::endl;
    std::cerr << "[groundtruth shape]: " << gt_pts << " " << gt_dim << std::endl;
    ...
    std::cerr << "[index file]: " << projection_index_save_file << std::endl;
    ...
    if (index.need_normalize) {
        ...
        for (uint32_t i = 0; i < q_pts; i++) {
            ...
            efanna2e::normalize<float>(query_data + i * q_dim, q_dim);
        }
    }
    ...
    for (uint32_t L_pq : L_vec) {
        ...
        // std::cout << "SEARCH L: " << L_pq << std::endl;
        std::cerr << "[search L]: " << L_pq << std::endl;
        // //pre test good
...
// #pragma omp parallel for schedule(dynamic, 1)
        for (size_t i = 0; i < q_pts; ++i) {
            ...
            auto ret_val = index.SearchRoarGraph(query_data + i * q_dim, k, i, parameters, res + i * k, res_dists[i]);
        ...
        // std::cout << "dist: " << avg_projection_cmps << std::endl;
        std::cerr << "[time]: " << (float)q_pts / ((float)time_us_per_query / 1000.0) << std::endl;
        std::cerr << "[dist]: " << avg_projection_cmps << std::endl;
        std::cerr << "[hop]: " << avg_hops << std::endl;
        std::cerr << "[recall@" << k << "]: " << recall * 100 << std::endl;
        // eval_recall(res, gt_load, K);

        
        // std::cout << "hop: " << avg_hops << std::endl;
